...
// Llenar la tabla con datos de series
var tvSeriesTable = document.getElementById('tv-series-table');
var totalSeasons = 0;
series.forEach(function (serie, index) {
    totalSeasons += serie.seasons;
    var row = document.createElement('tr');
    // Aplica 'table-active' a partir de la segunda fila
    if (index >= 0) {
        row.classList.add('table-active');
    }
    row.innerHTML = "\n      <th scope=\"row\">".concat(serie.id, "</th>\n      <td><a href=\"").concat(serie.url, "\" target=\"_blank\" rel=\"noopener noreferrer\">").concat(serie.name, "</a></td>\n      <td>").concat(serie.channel, "</td>\n      <td>").concat(serie.seasons, "</td>\n  ");
    tvSeriesTable.appendChild(row);
});
// Calcular y mostrar el promedio de temporadas
var averageSeasons = totalSeasons / series.length;
document.getElementById('average-seasons').textContent = "Average number of seasons: ".concat(averageSeasons.toFixed(0));
