...
series.forEach((serie, index) => {
  ...
  row.innerHTML = `
      ...
      <td><a href="${serie.url}" target="_blank" rel="noopener noreferrer">${serie.name}</a></td>
      <td>${serie.channel}</td>
      <td>${serie.seasons}</td>
  `;
...
document.getElementById('average-seasons')!.textContent = `Average number of seasons: ${averageSeasons.toFixed(0)}`;
